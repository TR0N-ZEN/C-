...
int main()
{
    ...
    */
    ...
    //------------------------------------------------------------
        //All members of a struct a by standard public, meaning they are accesible anywhere in the program.
        Vector v;
        v.x = 1;
        v.y = 2;
        v.z = -0.5;
    ...
    //const variables
    //------------------------------------------------------------
        const int M = 2; //'const int Q = 2' <=> 'int const Q = 2'
        //with pointers story is more complicated
        const int* N = &a; // 'const int* N = &a' <=> 'int const * N = &a'. As long as the keyword 'const' is located before the '*' the pointer itself can be changed, but not the value it points to 
        //if the keyword 'const' is written after the '*', the pointer itself cannnot be changed, but the value it points at
        int* const O = &a;
